...
├── assets/                  # Public README/Page assets
│   └── dotfiles-banner.svg
├── docs/                    # GitHub Pages source
│   ├── CNAME
│   └── index.html
├── wiki/                    # Source copy for GitHub wiki pages
│   ├── Home.md
│   ├── Installation.md
│   ├── Verification.md
│   ├── Customization.md
│   ├── Tmux.md
│   └── Vim.md
├── scripts/                 # Smoke verification and compatibility automation
│   ├── verify-suite.sh       # PASS/FAIL/SKIP wrapper for runtime contracts
│   ├── update-compat-matrix.sh
...
- Contains: Zsh configuration file `.zshrc`.
- Key files: `zsh/.zshrc` (active source).
...
**`scripts/`:**
- Purpose: Maintained automation for smoke verification and compatibility matrix updates.
- Contains: Shell/Python scripts explicitly allowlisted in `.gitignore`.
- Key files: `scripts/verify-suite.sh`, `scripts/update-compat-matrix.sh`, `scripts/update_compat_matrix.py`.
- Subdirectories: Not detected.

**`assets/`:**
- Purpose: Public branding assets for README and hosted docs.
- Contains: Operator Field Kit banner SVG.
- Key files: `assets/dotfiles-banner.svg`.

**`docs/`:**
- Purpose: GitHub Pages source for `https://dotfiles.teamoperator.red`.
- Contains: Static landing page and custom-domain file.
- Key files: `docs/index.html`, `docs/CNAME`.

**`wiki/`:**
- Purpose: Source copy for GitHub wiki pages.
- Contains: Markdown operational guides for install, verification, customization, tmux, and Vim.

## Key File Locations

**Entry Points:**
- `install.sh`: Bootstrap entry point for backing up and linking dotfiles.
- `scripts/verify-suite.sh`: Automated smoke verification entry point from repository root.
...
**Testing:**
- Maintained smoke wrapper: `scripts/verify-suite.sh`.
- No dedicated unit-test directories or test runner config files are present.
...
- `assets/dotfiles-banner.svg`: README banner asset.
- `docs/index.html`: GitHub Pages landing page.
- `docs/CNAME`: Custom domain for Pages.
- `wiki/*.md`: Source copy for GitHub wiki content.
...
- Backup/snapshot files use `*.bak`/`*.backup` suffixes and are ignored by `.gitignore`.
...
- Tests: Add or adjust focused `scripts/verify-suite.sh` checks where behavior can be validated non-interactively; add manual verification instructions in `README.md` or `AGENTS.md` for interactive-only behavior.
...
- Tests: Prefer wrapper checks for syntax/docs/contract invariants, plus focused runtime checks for interactive behavior.
...
- Tests: Prefer wrapper checks for syntax/docs/contract invariants, plus focused runtime checks for interactive command behavior.
